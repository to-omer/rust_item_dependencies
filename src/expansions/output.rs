    ...
    for &id in generated {
        ...
        let container = loop {
            ...
            if !generated.contains(&current) {
                // The owner may itself have been generated by another expansion.
                break DefinitionProductContainer::Owner(current);
            }
            let role =
                macro_definition_product_role(record).ok_or(ExpansionError::IncompleteOrigin)?;
            match role {
...
mod product_classification_tests {
    ...
    #[test]
    fn body_local_subordinates_can_have_an_owner_generated_by_another_expansion() {
        for kind in [
            ...
            let definitions = vec![
                written_owner(),
                expanded(1, kind, 0, None),
                expanded(2, DefinitionKind::Closure, 1, None),
                expanded(3, DefinitionKind::InlineConst, 2, None),
            ];
            let generated = [DefinitionId(2), DefinitionId(3)].into_iter().collect();
            let observed = BTreeMap::from([
                (DefinitionId(2), range(0, 4)),
                (DefinitionId(3), range(1, 2)),
            ]);
            let classified = classify_definition_products(&definitions, &generated, &observed)
                .expect("an external generated parent is the owner, not this producer's root");
            assert!(classified.products.is_empty());
            assert_eq!(classified.owner, Some(DefinitionId(1)));
            assert_eq!(classified.owner_members.len(), 2);
        }
    }

    #[test]
    fn subordinate_classification_rejects_incomplete_outputs() {
        let definitions = vec![
            written_owner(),
            expanded(1, DefinitionKind::Function, 0, None),
            expanded(2, DefinitionKind::Closure, 1, None),
    ...
    #[test]
    fn owner_members_require_a_unique_owner_and_their_own_output_evidence() {
        let definitions = vec![
            written_owner(),
            expanded(1, DefinitionKind::Function, 0, None),
            expanded(2, DefinitionKind::Closure, 1, None),
            expanded(3, DefinitionKind::Closure, 0, None),
            expanded(
                4,
        ...
        let generated = [DefinitionId(2), DefinitionId(3)].into_iter().collect();
        let observed = BTreeMap::from([
            (DefinitionId(2), range(0, 2)),
            (DefinitionId(3), range(2, 4)),
        ]);
        assert_eq!(
            classify_definition_products(&definitions, &generated, &observed).map(|_| ()),
            Err(ExpansionError::IncompleteOrigin)
        );
        let generated = [DefinitionId(4)].into_iter().collect();
        for observed in [
            BTreeMap::new(),
            BTreeMap::from([(DefinitionId(1), range(0, 4))]),
        ] {
            assert_eq!(
                classify_definition_products(&definitions, &generated, &observed).map(|_| ()),
                Err(ExpansionError::IncompleteOrigin),
                "a synthetic child cannot inherit output evidence from another producer",
            );
        }
    }
