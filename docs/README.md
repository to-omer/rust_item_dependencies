...
対象は、安定版Rustで書かれた、1ファイルの通常の実行プログラムまたはRustライブラリです。入力クレートの`#[link]`によるネイティブライブラリへのリンクを扱えます。ビルド済みのRustライブラリと、明示的に実行を許可した手続きマクロは依存先として指定できます。利用者が指定する外部クレートの成果物にネイティブリンク情報が含まれる場合は、対象外です。詳しい入力条件と削減ルールは[詳細仕様](specification.md)を参照してください。
