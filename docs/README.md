...
対象は、安定版Rustで書かれた、1ファイルの通常の実行プログラムまたはRustライブラリです。ビルド済みのRustライブラリと、明示的に実行を許可した手続きマクロは依存先として指定できます。別途ネイティブライブラリを必要とする依存先には対応していません。詳しい入力条件と削減ルールは[詳細仕様](specification.md)を参照してください。
