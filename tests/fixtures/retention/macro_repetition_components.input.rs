...
mod star_many {
    star_items!(dead_first => 1 => "a", kept_a => 10 => "b", dead_middle => 2 => "c", kept_b => 20 => "d", dead_last => 3 => "e");
}

mod star_only {
    star_items!(only_dead => 4 => "f");
}
...
mod plus_many {
    plus_items!(dead_first => 1, kept_a => 30, dead_middle => 2, kept_b => 40, dead_last => 3);
}
...
mod leading {
    leading_separator!(, dead_first, kept, dead_last);
}
...
mod trailing {
    trailing_separator!(dead_first, kept, dead_last,);
}
...
mod adjacent {
    no_separator!(dead_first kept dead_last);
}
